...
module.exports = async file => {
  ...
  const axis = startMarker.position.findIndex(
    (x, i) => x !== endMarker.position[i]
  );

  const delta = endMarker.position[axis] - startMarker.position[axis];

  // get the character plates
  const characters = parser
    .query({ asset: plateAsset, material: 'BMC_Plastic', color: 0 })
    // sort them based on distance from start marker
    .sort((a, b) => (a.position[axis] - b.position[axis]) * delta)
    // get the bricks above the plate
    .map(c => {
      const size =
        c.rotation === 1 || c.rotation == 3
          ? [c.size[1], c.size[0], c.size[2]]
          : c.size;
      return {
        bricks: moveBricks(
          parser.aboveBrick(c),
  ...
  if (characters.length !== 95) throw new Error('not enough characters');

  return {
    // generate brick text from string
    text: (
      str = '',
      { shift = [0, 0, 0], color = [0, 0, 0], author, centered = false }
    ) => {
      ...
      const fullWidth = chars.reduce(
        (a, c) => a + characters[c - 32].width + spacing * 2,
        -spacing
      );
      if (centered) length -= fullWidth / 2;
