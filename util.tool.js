...
const indexify = (arr, key) =>
  Object.fromEntries(
    Object.entries(arr || []).map(a => [key ? a[1][key] : a[1], Number(a[0])])
  );
...
class ParseTool {
  ...
  query(
    ...
    return (bricks || this.save.bricks).filter(
      b =>
        (!size || eq(b.size, size)) &&
  ...
  aboveBrick(b) {
    const size =
      b.rotation === 1 || b.rotation == 3
        ? [b.size[1], b.size[0], b.size[2]]
        : b.size;

    const min = [
      b.position[0] - size[0] - 2,
      b.position[1] - size[1] - 2,
      b.position[2] + 1,
    ];
    const max = [
      b.position[0] + size[0] + 2,
    ...
    return this.save.bricks.filter(b =>
      b.position.every((p, i) => p >= min[i] && p <= max[i])
    );
  }
  ...
  betweenBricks(query, bricks) {
    ...
    const min = [
      Math.min(markers.map(b => b.position[0])),
      Math.min(markers.map(b => b.position[1])),
      0,
    ];
    const max = [
      Math.max(markers.map(b => b.position[0])),
      Math.max(markers.map(b => b.position[1])),
      Infinity,
    ];
...
class WriteTool {
  constructor(save) {
    ...
    this.names =
      save && save.brick_owners ? indexify(save.brick_owners, 'name') : {}; // both of these are off by 1 due to 1-indexing
    this.ids =
      save && save.brick_owners ? indexify(save.brick_owners, 'id') : {}; // both of these are off by 1 due to 1-indexing
  ...
  findAsset(name) {
    if (typeof this.assets[name] !== 'undefined') return this.assets[name];
  ...
  brick({
    ...
    return {
      ...
      color,
      material_index:
        typeof material === 'number' ? material : this.findMaterial(material),
      size,
      rotation,
      direction,
      position,
  ...
  write() {
    ...
    // overwrite ownership
    if (authors.length === 0) for (const b of this.bricks) b.owner_index = 1;
